...
int main(int argc, char* argv[])
{
    ...
    try {
        ...
        options.add_options()("help,h", "Show this message")(
            ...
            "preset,p", boost::program_options::value<int>(&selectedPreset)
                            ->default_value(0),
        ...
        // Transfer the parsed strings into the parsedFiles vector for easier
        // access
        if (parsedOptions.count("input-file")) {
            parsedFiles =
                parsedOptions["input-file"].as<std::vector<std::string>>();
        } else {
    ...
    while (filesIterator != filteredFileVector.end()) {
        ...
        ffdropenc::Video newVideo(
            *filesIterator, preset_list[selectedPreset], imgSeq);
    ...
    std::vector<ffdropenc::Video>::iterator duplicateRemover =
        std::unique(queue.begin(), queue.end());
    queue.resize(std::distance(queue.begin(), duplicateRemover));

    for (std::vector<ffdropenc::Video>::iterator queueItem = queue.begin();
         queueItem != queue.end(); ++queueItem)
        queueItem->transcode();

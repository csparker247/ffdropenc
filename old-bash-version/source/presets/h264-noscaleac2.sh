...
# optname H.264 Generic (No Scale, Square Pixels, 1st Two Audio Channels)
...
for (( i=1; i<=${args}; i++ )); do
		...
		# Video pass
			...
			-c:v "$VENCODER" -crf "$CRF" -maxrate "$TARGET_VRATE" -bufsize "$TARGET_BUFFER" -pix_fmt "$PIX_FMT" \
				-vf \
					"scale=iw*sar:ih,
					scale=
					...
					setsar=1" \
				-movflags faststart \
			-map_channel 0.1.0 -map_channel 0.1.1 -c:a libfdk_aac -b:a "$TARGET_ARATE" \
			-y "$OUTFILE" \
			2>&1 | awk '1;{fflush()}' RS='\r\n'>"$ERRLOG" &

...
for (( i=1; i<=${args}; i++ )); do
		...
		# Video pass
			...
			ENCODER="FFMPEG"
			ffmpeg $(echo $SEQ_OPTS) -i "${filelist[$index]}" -map 0 -c copy -c:v libx264 -crf 16 -maxrate 45M -bufsize 40M -pix_fmt yuv420p -vf "scale=iw*sar:ih, idet, w3fdif=complex:interlaced" -y "$OUTFILE" \
			2>&1 | awk '1;{fflush()}' RS='\r\n'>"$ERRLOG" &

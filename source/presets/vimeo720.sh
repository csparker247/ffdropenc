...
for (( i=1; i<=${args}; i++ ));
	do
		# Remove the extension and make filenames for logs and output.
			...
			ERRLOG="$RAWNAME"_Vimeo720-debug.log
		...
		# Video pass
			...
			ENCODER="FFMPEG"
			ffmpeg -i "${filelist[$i]}" -c:v libx264 -b:v 5M -pix_fmt yuv420p -profile:v baseline -vf scale=1280:-1 -c:a libfdk_aac -b:a 320k -ar 44.1k -y "$OUTFILE" \
			2>&1 | awk '1;{fflush()}' RS='\r\n'>"$ERRLOG" &

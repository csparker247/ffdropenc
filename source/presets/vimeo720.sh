...
for (( i=1; i<=${args}; i++ )); do
		...
		# Video pass
			...
			-c:v libx264 -crf 18 -maxrate 8M -bufsize 16M -profile:v high -level 42 -pix_fmt yuv420p -vf "scale=iw*sar:ih, scale='if(gt(iw,ih),min(1280,iw),-1)':'if(gt(iw,ih),-1,min(720,ih))'" -movflags faststart \

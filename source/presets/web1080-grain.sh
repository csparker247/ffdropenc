...
for (( i=1; i<=${args}; i++ )); do
		...
		# Video pass
			...
			ENCODER="FFMPEG"
			ffmpeg $(echo $SEQ_OPTS) -i "${filelist[$index]}" -c:v libx264 -b:v 10M -pix_fmt yuv420p -preset slow -tune grain -profile:v baseline -vf scale=1920:-1 -c:a libfdk_aac -b:a 320k -ar 44.1k -y "$OUTFILE" \
			2>&1 | awk '1;{fflush()}' RS='\r\n'>"$ERRLOG" &

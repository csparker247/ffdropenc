...
for (( i=1; i<=${args}; i++ )); do
		...
		# Video pass
			...
			-c:v libx264 -crf 16 -maxrate 45M -bufsize 65M -profile:v high -level 42 -pix_fmt yuv420p -vf "scale=iw*sar:ih, scale='if(gt(iw,ih),min(1920,iw),-1)':'if(gt(iw,ih),-1,min(1080,ih))'" -movflags faststart \

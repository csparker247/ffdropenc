...
for (( i=1; i<=${args}; i++ ));
	do
		...
		# YouTube Pass
			...
			ffmpeg -i "${filelist[$i]}" -pass 2 "$TWOPASS" -c:v libx264 -b:v 45M -maxrate 55M -bufsize 3M -pix_fmt yuv420p -vf scale=1920:-1 -c:a libfdk_aac -profile:a aac_low -b:a 320k -ar 48k -y "$YOUFILE" 2>"$YERRLOG"2
				# Progress update
				count=$(echo "scale=3; $count+0.4" | bc)
				PROG=$(echo "scale=3; ($count/$args)*100.0" | bc)
				echo PROGRESS:"$PROG"

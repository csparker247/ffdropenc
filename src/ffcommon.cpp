...
namespace ffdropenc {
    ...
    bool isImgSequence( boost::filesystem::path file ) {
        ...
        for ( int i = 0; i < FF_IMG_EXTENSIONS.size(); ++i ) {
            if ( extension == "." + FF_IMG_EXTENSIONS[i] ) return FF_IS_IMG_SEQ;
        }
    ...
    void RemoveDirs(std::vector<std::string> &fileList, std::vector<std::string> &resolvedFiles) {
        ...
        while ( file != fileList.end() ) {
            ...
            if (!boost::filesystem::exists( *file)) continue;
            ...
            else if ( boost::filesystem::is_regular_file( *file)) resolvedFiles.push_back(*file);
                ...
                while (dir != dir_end) {
                    ...
                    if ( is_regular_file( dir_entry )) resolvedFiles.push_back(dir_entry.string());
                    ++dir;
                }
